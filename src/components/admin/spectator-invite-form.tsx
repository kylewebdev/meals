...
export function SpectatorInviteForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await createInvite({ email, role: 'spectator' });

    if (res.success) {
      setResult({ url: res.data.inviteUrl });
      setEmail('');
      toast('Spectator invite created');
    } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      <div>
        <Label htmlFor="spectator-email">Invite spectator by email</Label>
